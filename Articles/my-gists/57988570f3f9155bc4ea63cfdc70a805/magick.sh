# create watermark
magick convert -size 500x500 xc:none -font 'Freestyle-Script' -pointsize 50 -draw "gravity center fill rgba(0,0,0,0.5) rotate -30 text 0,0 '@TokumeiAsia'" watermark.png
...
## yellow tape
magick convert input.png \( -pointsize 30 -background none -undercolor '#ffff0040' -fill black -font 'Freestyle-Script' -gravity center label:"@TokumeiAsia @TokumeiAsia @TokumeiAsia @TokumeiAsia @TokumeiAsia @TokumeiAsia " -trim -rotate -30 -bordercolor none -border 0 -write mpr:wm +delete +clone -fill mpr:wm  -draw 'color 0,0 reset' \) -compose over -composite out_watermark.png
